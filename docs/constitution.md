...
- **Statement:** Serialized JSON uses PascalCase. Do not add `[JsonPropertyName]` unless the serialized name genuinely must differ (e.g. CosmosDB `id`, `ttl`, partition key paths). **Exception:** RFC 7807 error payloads (`ProblemDetails` and any `*ProblemDetails` schema) use the lowercase member names the RFC fixes (`type`, `title`, `status`, `detail`, `instance`), with lowerCamelCase extension members. See `docs/adrs/0011-rfc7807-error-payload-casing.md`.
- **Rationale:** All existing API and Portal consumers are locked to PascalCase; changing the wire format is a breaking change across the surface. Deliberate deviation from the Zalando snake_case MUST (see `docs/conventions/api.md`). The error-payload exception exists because the framework's `ProblemDetails` hardcodes lowercase `[JsonPropertyName]` attributes for `application/problem+json` conformance, and those attributes override any naming policy.
- **Enforcement:** code review (manual) plus `Check-ApiPascalCase.ps1` (advisory CI gate; flags non-PascalCase `[JsonPropertyName]` wire names, excludes external-integration DTOs and `*ProblemDetails` spec schemas) - active; deferred (planned: flip to blocking after the flagged slips are resolved). Known blind spot: the gate inspects `[JsonPropertyName]` attributes only and cannot see the serializer's `PropertyNamingPolicy`, which is how the camelCase regression fixed in ADR 0011 went undetected. Code-vs-spec OpenAPI drift is NOT covered (it needs build-time swagger generation).
- **Added:** 2026-06-02  **Supersedes:** none
...
- 2026-08-31 - v1.4 - Amended C06 with an explicit RFC 7807 exception for error payloads, recorded in ADR 0011. Context: the API had been emitting camelCase on every endpoint because `AddControllers()` inherits `JsonSerializerDefaults.Web` (camelCase naming policy) and nothing overrode it; the C06 gate could not see this because it inspects `[JsonPropertyName]` attributes only. Fixing the naming policy exposed the unresolvable conflict between C06 and the framework's `ProblemDetails`, whose lowercase member names are fixed by the RFC. C06's scope is now the domain surface; error payloads follow RFC 7807. Enforcement field updated to record the gate's naming-policy blind spot.
