...
public class MigrationTests : IAsyncLifetime
{
    ...
    [Trait("Story", "MN-17")]
    [Trait("AC", "AC-8")]
    public async Task AddSessionsMigration_CreatesSessionsTable()
    {
        var options = new DbContextOptionsBuilder<MindNovaDbContext>()
            .UseSqlServer(_sqlContainer.GetConnectionString())
            .Options;
        ...
        await connection.OpenAsync();
        await using var command = connection.CreateCommand();
        command.CommandText = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'Sessions'";
        var result = await command.ExecuteScalarAsync();
    ...
    [Trait("Story", "MN-17")]
    [Trait("AC", "AC-9")]
    public async Task AddSessionsMigration_AppliesCleanly()
    {
        var options = new DbContextOptionsBuilder<MindNovaDbContext>()
            .UseSqlServer(_sqlContainer.GetConnectionString())
            .Options;
